...
- 所有通路对的 Jaccard 值均很低（最高 ICF-TT @50 仅 7.6%），说明四路通路具有高度互补性
...
3. **四路候选集高度互补**：最高 Jaccard@50 仅 7.6%（ICF vs TT），其余通路对均低于 4%，说明多路融合带来的覆盖面远超单一通路。

4. **ICF + TwoTower 是核心贡献通路**：二者合计贡献 RRF 得分的 95.4%，命中归因（分数加权）超过 79.7%。Text 和 Pop 以较低权重贡献约 20% 的分数命中。

5. **Text 和 Pop 的融合机制为 re-ranking 而非新候选**：两者在 top-200 口径下独占命中为 0，说明其命中 item 均已被 ICF/TT 覆盖，融合收益来自 RRF 得分强化（推动 borderline items 进入 top-50），而非引入 ICF/TT 无法发现的新候选。
