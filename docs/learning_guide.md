...
3. Text Semantic：MiniLM 文本语义信号，作为低权重 RRF 重排序先验
4. Popularity fallback：低权重热门先验，作为 RRF 重排序先验
```
...
召回层真正互补的是 ICF + TT 两路；Text/Pop 在 top-200 口径下独占命中为 0，
更准确地说是帮助 RRF 把 ICF/TT 已覆盖的 borderline items 推进 top-50 的重排序先验。
```
...
| `LogQ` | 采样概率修正 | 训练时用 `corrected_logit = s - log q` 修正 item 采样偏差；它不是流行度打压器。 |
...
1. **效果**：真实目标是否出现在候选集。
2. **覆盖**：是否只会推荐少量热门物品。
3. **互补性**：不同通路是否命中不同类型的真实目标；本项目召回层真正互补的是 ICF + TT，两路之外的 Text/Pop 主要是 RRF 重排序先验。
...
| Text Semantic | 文本语义 | 在最终 4ch 中更多作为低权重 RRF 重排序先验 |
| Popularity | 全局热度 | 在最终 4ch 中更多作为低权重 RRF 重排序先验 |
...
```text
召回层真正互补的是 ICF + Transformer TT 两路。
Text Semantic / Popularity 在 top-200 口径下独占命中为 0，
它们对最终 Recall 的贡献来自 RRF 得分强化，而不是引入 ICF/TT 找不到的新命中。
```
...
如果写成单个 user-item 分数，就是：

```text
corrected_logit = s - log q
```
...
这一步是理解 LogQ 负结果的关键。训练时热门 item 的 `q` 更大，`log q` 也更大；
`s - log q` 会压低热门 item 在训练 softmax 里的有效 logit。模型如果仍想让这些热门正样本被正确分类，
就可能学习到更高的原始打分 `s` 来补偿这项扣分。推理阶段不再减 `log q`，只用 raw dot-product，
于是这些被补偿抬高的热门 item 反而更容易被召回。

所以 LogQ 是采样偏差纠偏（sampling-bias correction），不是流行度打压（popularity suppression）旋钮。
它可能让训练目标更接近被采样前的 softmax，但不保证最终曝光更均衡。
...
因为推理阶段使用 raw dot-product，训练期被 `-log q` 压过的热门 item 可能通过更高原始打分补偿回来。
...
补充边界：最终 4ch 系统中，召回层真正互补的是 ICF + TT。Text/Pop 在持久化候选 top-200 审计中独占命中为 0，
因此不要把它们说成“新增独立命中的召回通路”；更准确的说法是 RRF 重排序先验。

### Q5：Text Semantic 单路这么弱，为什么保留？

```text
它不是主路。最终审计显示 Text/Pop 在 top-200 口径下独占命中为 0，
所以不能把它讲成带来独立新命中的召回路。
它的作用更像 RRF 重排序先验：给 ICF/TT 已覆盖的候选补充语义信号，把 borderline items 推进 top-50。
```
...
corrected_logit = s - log q。
这会在训练 softmax 里压低更常被采样的热门 item，
但模型可能通过抬高 raw score s 来补偿；推理时又只用 raw score，
所以热门 item 可能反而更容易被召回。
因此 LogQ 是采样偏差纠偏，不是流行度打压。
```
...
11. LogQ 使用 `corrected_logit = s - log q` 修正训练采样偏差；它不是流行度打压旋钮，强修正可能让模型抬高热门 item 的 raw dot-product 来补偿。
