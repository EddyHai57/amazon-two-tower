#!/usr/bin/env python3
"""Semi-hard Negative Mining smoke on Text + Mean Pooling Two-Tower.
...
(loaded from checkpoint), but are selected from an INTERMEDIATE similarity
band rather than the top nearest neighbors.

Motivation: model-based top-50 HNM (epoch1 Recall@50=0.105200, below baseline
0.107460) suggests that taking the very nearest neighbors as hard negatives is
too disruptive at epoch 1 — those candidates may be false negatives or their
gradient direction conflicts with the main in-batch loss.

Semi-hard strategy:
  - Search top hn_top_k_search (e.g. 300) neighbors per item via Faiss.
  - Exclude self.
  - Skip rank [0, semi_hard_start_rank) = the hardest candidates.
  - Use rank [semi_hard_start_rank, semi_hard_end_rank) as the semi-hard pool.
  - Ranks are 0-indexed (rank 0 = most similar neighbor after self-exclusion).

Steps:
  1. Load final-model checkpoint → export all L2-normalized item embeddings.
  2. Build Faiss IndexFlatIP, search top hn_top_k_search per item, slice
     semi-hard band → semi-hard candidate table.
...
REQUIRED_CONFIG_KEYS = [
    ...
    "model_hn_checkpoint", "lambda_hn", "hard_negatives_per_sample",
    "hn_top_k_search", "semi_hard_start_rank", "semi_hard_end_rank",
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Semi-hard HNM smoke: Text + Mean Pooling Two-Tower.")
...
def require_config(config: dict[str, Any]) -> None:
    ...
    start = int(config["semi_hard_start_rank"])
    end = int(config["semi_hard_end_rank"])
    search = int(config["hn_top_k_search"])
    if start < 0:
        raise ValueError("semi_hard_start_rank must be >= 0.")
    if end <= start:
        raise ValueError("semi_hard_end_rank must be > semi_hard_start_rank.")
    if search < end:
        raise ValueError("hn_top_k_search must be >= semi_hard_end_rank.")
    n_candidates = end - start
    if n_candidates < int(config["hard_negatives_per_sample"]):
        raise ValueError(
            f"semi_hard band width {n_candidates} < hard_negatives_per_sample "
            f"{config['hard_negatives_per_sample']}."
        )
...
# ---------------------------------------------------------------------------
# Step 2: Build semi-hard negative table with Faiss IndexFlatIP
# ---------------------------------------------------------------------------

def build_semi_hard_negative_table(
    item_emb_np: np.ndarray,
    top_k_search: int,
    semi_hard_start_rank: int,
    semi_hard_end_rank: int,
) -> np.ndarray:
    """Find semi-hard negative candidates for each item using Faiss IndexFlatIP.

    item_emb_np is L2-normalized, so inner product == cosine similarity.
    Self is excluded from each row's neighbor list.

    Semi-hard candidates = neighbors at rank [semi_hard_start_rank, semi_hard_end_rank)
    after excluding self. Ranks are 0-indexed: rank 0 = most similar neighbor.

    Skipping rank [0, semi_hard_start_rank) avoids the hardest near-duplicates.
    Using rank [semi_hard_start_rank, semi_hard_end_rank) gives candidates that
    are still semantically similar but less likely to be false negatives or to
    produce conflicting gradients with the main in-batch loss.

    Returns hn_table of shape (n_items, n_candidates_per_item) with dtype int32,
    where n_candidates_per_item = semi_hard_end_rank - semi_hard_start_rank.
    -1 entries indicate positions where fewer candidates were available.
    """
    import faiss  # local import: optional dependency, verified available

    n_items, embedding_dim = item_emb_np.shape
    n_candidates = semi_hard_end_rank - semi_hard_start_rank
    logging.info(
        "Building semi-hard HN table: n_items=%d embedding_dim=%d "
        "top_k_search=%d rank_band=[%d,%d) n_candidates_per_item=%d (Faiss IndexFlatIP)",
        n_items, embedding_dim, top_k_search,
        semi_hard_start_rank, semi_hard_end_rank, n_candidates,
    )
    t0 = time.time()
    index = faiss.IndexFlatIP(embedding_dim)
    index.add(item_emb_np)
    # Search top_k_search + 2 to have slack for excluding self
    _D, I = index.search(item_emb_np, top_k_search + 2)
    del index, _D

    hn_table = np.full((n_items, n_candidates), -1, dtype=np.int32)
    for i in range(n_items):
        # Exclude self, keep valid neighbors in similarity order
        non_self = [int(j) for j in I[i] if int(j) != i and int(j) >= 0]
        # Slice semi-hard band: skip top semi_hard_start_rank, take up to semi_hard_end_rank
        semi_hard = non_self[semi_hard_start_rank:semi_hard_end_rank]
        hn_table[i, : len(semi_hard)] = semi_hard

    elapsed = time.time() - t0
    valid_entries = int((hn_table >= 0).sum())
    coverage = 100.0 * valid_entries / (n_items * n_candidates)
    logging.info(
        "Semi-hard HN table built in %.2fs: shape=%s valid_entries=%d (%.1f%%)",
        elapsed, hn_table.shape, valid_entries, coverage,
    )
    return hn_table
...
def train_model_semi_hnm_smoke(config: dict[str, Any]) -> None:
    ...
    semi_hard_start_rank = int(config["semi_hard_start_rank"])
    semi_hard_end_rank = int(config["semi_hard_end_rank"])
    n_candidates_per_item = semi_hard_end_rank - semi_hard_start_rank

    logging.info(
        "device=%s embedding_dim=%s batch_size=%s lr=%s temperature=%s "
        "lambda_hn=%s hard_negatives_per_sample=%s "
        "hn_top_k_search=%s semi_hard_start_rank=%s semi_hard_end_rank=%s n_candidates_per_item=%d",
        ...
        config["hn_top_k_search"], semi_hard_start_rank, semi_hard_end_rank, n_candidates_per_item,
    )
    ...
    # ── Step 2: Build semi-hard HN table ──────────────────────────────────
    hn_table = build_semi_hard_negative_table(
        item_emb_np,
        top_k_search=int(config["hn_top_k_search"]),
        semi_hard_start_rank=semi_hard_start_rank,
        semi_hard_end_rank=semi_hard_end_rank,
    )
    valid_entries = int((hn_table >= 0).sum())
    hn_table_coverage = valid_entries / (len(hn_table) * n_candidates_per_item)
    del item_emb_np
    ...
    for epoch in range(1, int(config["epochs"]) + 1):
        ...
        epoch_results[epoch] = {
            ...
            "hn_coverage_ratio": float(total_valid_hn) / max(len(bundle.train_df), 1),
    ...
    write_json(output_dir / "semi_hnm_smoke_results.json", epoch_results)
    summary = {
        "model_hn_checkpoint": str(checkpoint_path),
        "hn_top_k_search": int(config["hn_top_k_search"]),
        "semi_hard_start_rank": semi_hard_start_rank,
        "semi_hard_end_rank": semi_hard_end_rank,
        "n_candidates_per_item": n_candidates_per_item,
        "hn_table_coverage": hn_table_coverage,
        "lambda_hn": float(config["lambda_hn"]),
        "hard_negatives_per_sample": int(config["hard_negatives_per_sample"]),
    ...
    logging.info("Semi-hard HNM smoke complete: %s", json.dumps(summary, ensure_ascii=False, sort_keys=True))
...
def main() -> None:
    ...
    train_model_semi_hnm_smoke(config)
