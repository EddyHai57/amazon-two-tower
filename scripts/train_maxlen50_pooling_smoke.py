#!/usr/bin/env python3
"""Max-len 50 paired smoke — time_decay / time_aware_attention / gated pooling.

Three pooling variants, same settings, max_len=50, 3 epochs.
History bucket analysis uses raw (pre-truncation) lengths so the >20 bucket
is populated even when history_max_len < actual history lengths.

Pooling variants
----------------
time_decay          : exponential decay weighted mean (same as final model, max_len=50)
time_aware_attention: scaled dot-product attention with log-decay positional bias;
                      degrades to time_decay when attention scores are uniform
gated               : learnable gate blending time_decay pool and pure attention pool
                      gate = sigmoid(W_g · user_id_emb), +64 params vs time_decay

Usage
-----
Training (one at a time):
  python scripts/train_maxlen50_pooling_smoke.py --config configs/maxlen50_smoke_td.yaml
  python scripts/train_maxlen50_pooling_smoke.py --config configs/maxlen50_smoke_attention.yaml
  python scripts/train_maxlen50_pooling_smoke.py --config configs/maxlen50_smoke_gated.yaml

Comparison (after all three training runs complete):
  python scripts/train_maxlen50_pooling_smoke.py --compare \\
    --td_dir    outputs/attention_pooling_maxlen50_smoke/time_decay \\
    --attn_dir  outputs/attention_pooling_maxlen50_smoke/time_aware_attention \\
    --gated_dir outputs/attention_pooling_maxlen50_smoke/gated \\
    --out_dir   outputs/attention_pooling_maxlen50_smoke
"""
...
VALID_POOLING = ("time_decay", "time_aware_attention", "gated")
...
FNAME_MAP = {
    "time_decay":           "time_decay_max50_metrics.json",
    "time_aware_attention": "time_aware_attention_max50_metrics.json",
    "gated":                "gated_attention_max50_metrics.json",
}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--compare",    action="store_true", help="Run 3-way comparison.")
    p.add_argument("--td_dir",     default="outputs/attention_pooling_maxlen50_smoke/time_decay")
    p.add_argument("--attn_dir",   default="outputs/attention_pooling_maxlen50_smoke/time_aware_attention")
    p.add_argument("--gated_dir",  default="outputs/attention_pooling_maxlen50_smoke/gated")
    p.add_argument("--out_dir",    default="outputs/attention_pooling_maxlen50_smoke")
    return p.parse_args()
...
def compute_raw_history_lengths(train_df: pd.DataFrame, n_users: int) -> np.ndarray:
    """Actual per-user history length BEFORE any max_len truncation.

    Used exclusively for bucket assignment in evaluation so the >20 bucket
    reflects users with >20 actual train interactions.
    """
...
def load_text_artifacts(cfg: dict[str, Any]) -> tuple[torch.Tensor, torch.Tensor]:
    text_emb = torch.from_numpy(np.load(Path(cfg["item_text_embedding_path"])).astype(np.float32))
    has_text  = torch.from_numpy(np.load(Path(cfg["item_has_text_path"])).astype(np.float32))
...
class TextTwoTowerMaxlen50(nn.Module):
    """Two-Tower supporting three user-tower pooling variants.

    Item tower identical across all variants.
    time_decay          : exponential decay weighted mean
    time_aware_attention: attention with log-decay positional bias
                          (flat attention → identical to time_decay)
    gated               : sigmoid(W_g·user_id_emb) blends td + pure attention
                          (+64 trainable params vs other variants)
    """
    ...
    def __init__(
        ...
        # Gated only: W_g maps user_id_emb → scalar gate in (0,1).
        # Zero-init → sigmoid(0)=0.5, equal initial blend of td and attention.
        if self.pooling_type == "gated":
            self.gate_proj = nn.Linear(embedding_dim, 1, bias=False)
            nn.init.zeros_(self.gate_proj.weight)

    # ── pooling helpers ──────────────────────────────────────────────────────

    def _time_decay_pool(
        self,
        ...
        w    = self.decay_rate ** (L - 1 - pos)           # (L,)  newest=1.0
    ...
    def _pure_attention_pool(
        self,
        hist_emb: torch.Tensor,
        valid_mask: torch.Tensor,
        user_emb: torch.Tensor,   # (B, dim)  raw user_id_emb
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Scaled dot-product attention, no time bias; query = user_id_emb."""
        ...
        return torch.bmm(attn_w.unsqueeze(1), hist_emb).squeeze(1), attn_w

    def _time_aware_attention_pool(
        self,
        hist_emb: torch.Tensor,
        valid_mask: torch.Tensor,
        user_emb: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Attention with additive log-decay positional bias.

        Bias_k = (L-1-k) * log(decay_rate)   (newest=0, oldest≈-10.9 at L=50)
        When attention scores are uniform, softmax(bias) = time_decay weights.
        Attention can overcome bias only for highly relevant non-recent items.
        """
        L      = hist_emb.shape[1]
        scale  = math.sqrt(hist_emb.shape[-1])
        ...
        pos       = torch.arange(L, device=hist_emb.device, dtype=scores.dtype)
        time_bias = (L - 1 - pos) * math.log(self.decay_rate)  # (L,) ≤ 0
        scores    = scores + time_bias.unsqueeze(0)
    ...
    def _pool_history(
        ...
        if self.pooling_type == "time_decay":
            return self._time_decay_pool(hist_emb, valid_mask), None

        if self.pooling_type == "time_aware_attention":
            pooled, attn_w = self._time_aware_attention_pool(hist_emb, valid_mask, user_emb)
            return pooled, attn_w

        # gated: blend time_decay and pure_attention via learned scalar gate
        td_pooled            = self._time_decay_pool(hist_emb, valid_mask)
        attn_pooled, attn_w  = self._pure_attention_pool(hist_emb, valid_mask, user_emb)
        gate                 = torch.sigmoid(self.gate_proj(user_emb))  # (B, 1)
        return gate * td_pooled + (1.0 - gate) * attn_pooled, attn_w
...
def build_model(
...
) -> TextTwoTowerMaxlen50:
    text_emb, has_text = load_text_artifacts(cfg)
    model = TextTwoTowerMaxlen50(
    ...
    logging.info("TextTwoTowerMaxlen50 params=%s  pooling=%s  max_len=%s",
...
# ---------------------------------------------------------------------------
# Evaluation — uses raw_history_lengths for correct bucket assignment
# ---------------------------------------------------------------------------

def encode_all_items(
    model: TextTwoTowerMaxlen50, num_items: int, device: torch.device
) -> torch.Tensor:
    model.eval()
    chunks = []
    with torch.no_grad():
...
def check_sanity(
    ...
    """Log and return attention/gate statistics for non-time_decay variants."""
    ptype = model.pooling_type
    if ptype == "time_decay":
        return {}
    ...
    model.eval()
    sanity: dict[str, Any] = {"pooling_type": ptype, "sample_size": n_sample}
    with torch.no_grad():
        id_emb     = model.user_embedding(user_t)
        valid_mask = hist_t >= 0
        safe       = hist_t.clamp_min(0)
        hist_emb   = model.item_id_embedding(safe)

        if ptype == "gated":
            gate_vals = torch.sigmoid(model.gate_proj(id_emb)).squeeze(-1)  # (N,)
            sanity["gate_mean"]         = float(gate_vals.mean())
            sanity["gate_min"]          = float(gate_vals.min())
            sanity["gate_max"]          = float(gate_vals.max())
            sanity["pct_td_preferred"]  = float((gate_vals > 0.5).float().mean())
            logging.info("[GateSanity] mean=%.4f min=%.4f max=%.4f pct_td_preferred=%.3f",
                         sanity["gate_mean"], sanity["gate_min"], sanity["gate_max"],
                         sanity["pct_td_preferred"])
            _, attn_w = model._pure_attention_pool(hist_emb, valid_mask, id_emb)
        else:
            _, attn_w = model._time_aware_attention_pool(hist_emb, valid_mask, id_emb)

        if attn_w is not None:
            has_hist = valid_mask.any(dim=1)
            sum_per  = (attn_w * valid_mask.float()).sum(dim=1)
            sums     = sum_per[has_hist]
            nan_c    = int(torch.isnan(attn_w).sum())
            inf_c    = int(torch.isinf(attn_w).sum())
            w_max    = float(attn_w.max())
            w_min    = float(attn_w[valid_mask].min()) if valid_mask.any() else 0.0
            s_mean   = float(sums.mean()) if sums.numel() > 0 else 0.0
            sanity.update({
                "nan_count": nan_c, "inf_count": inf_c,
                "attn_sum_mean": s_mean,
                "attn_sum_min":  float(sums.min())  if sums.numel() > 0 else 0.0,
                "attn_sum_max":  float(sums.max())  if sums.numel() > 0 else 0.0,
                "attn_weight_max": w_max, "attn_weight_min": w_min,
            })
            logging.info("[AttnSanity/%s] NaN=%d Inf=%d sum_mean=%.4f w_max=%.4f",
                         ptype, nan_c, inf_c, s_mean, w_max)
    return sanity
...
def train(cfg: dict[str, Any]) -> None:
    ...
    n_gt50    = int((raw_lens > 50).sum())
    logging.info("raw history: non-empty=%d avg=%.2f pct_gt20=%.1f%% pct_gt50=%.1f%%",
                 int((raw_lens > 0).sum()), float(raw_lens.mean()),
                 100.0 * n_gt20 / n_users, 100.0 * n_gt50 / n_users)
    ...
    # Sanity check for non-time_decay variants
    if cfg["pooling_type"] != "time_decay":
        sanity = check_sanity(model, bundle.valid_df, hist_mat, device)
...
def compare(
    td_dir: Path, attn_dir: Path, gated_dir: Path, out_dir: Path
) -> dict[str, Any]:
    out_dir.mkdir(parents=True, exist_ok=True)

    td_s,    td_hits    = load_run(td_dir)
    attn_s,  attn_hits  = load_run(attn_dir)
    gated_s, gated_hits = load_run(gated_dir)
    ...
    result: dict[str, Any] = {
        ...
        "attn_recall@50":         attn_s["best_valid_recall@50"],
        "gated_recall@50":        gated_s["best_valid_recall@50"],
        "attn_vs_td_delta":       attn_s["best_valid_recall@50"]  - td_s["best_valid_recall@50"],
        "gated_vs_td_delta":      gated_s["best_valid_recall@50"] - td_s["best_valid_recall@50"],
        "meets_threshold_attn":   (attn_s["best_valid_recall@50"]  - td_s["best_valid_recall@50"]) >= 0.001,
        "meets_threshold_gated":  (gated_s["best_valid_recall@50"] - td_s["best_valid_recall@50"]) >= 0.001,
        # NDCG / MRR
        "td_ndcg@50":    td_s["best_valid_ndcg@50"],
        "attn_ndcg@50":  attn_s["best_valid_ndcg@50"],
        "gated_ndcg@50": gated_s["best_valid_ndcg@50"],
        "td_mrr@50":     td_s["best_valid_mrr@50"],
        "attn_mrr@50":   attn_s["best_valid_mrr@50"],
        "gated_mrr@50":  gated_s["best_valid_mrr@50"],
        # Best epochs / train time
        "td_best_epoch":          td_s["best_epoch"],
        "attn_best_epoch":        attn_s["best_epoch"],
        "gated_best_epoch":       gated_s["best_epoch"],
        "td_total_train_sec":     td_s["total_train_sec"],
        "attn_total_train_sec":   attn_s["total_train_sec"],
        "gated_total_train_sec":  gated_s["total_train_sec"],
        # Bucket breakdown
        "td_bucket_recall@50":    td_s["bucket_recall@50"],
        "td_bucket_counts":       td_s["bucket_counts"],
        "attn_bucket_recall@50":  attn_s["bucket_recall@50"],
        "gated_bucket_recall@50": gated_s["bucket_recall@50"],
        "bucket_delta_attn_vs_td":  bdelta(attn_s,  td_s),
        "bucket_delta_gated_vs_td": bdelta(gated_s, td_s),
        # Unique hit set analysis
        "num_eval_users":         td_s["num_eval_users"],
        "td_total_hits":          len(td_hits),
        "attn_total_hits":        len(attn_hits),
        "gated_total_hits":       len(gated_hits),
        "all_three_hit":          len(td_hits & attn_hits & gated_hits),
        "only_td":                len(td_hits  - attn_hits  - gated_hits),
        "only_attn":              len(attn_hits - td_hits   - gated_hits),
        "only_gated":             len(gated_hits - td_hits  - attn_hits),
        "td_and_attn_not_gated":  len((td_hits & attn_hits)  - gated_hits),
        "td_and_gated_not_attn":  len((td_hits & gated_hits) - attn_hits),
        "attn_and_gated_not_td":  len((attn_hits & gated_hits) - td_hits),
    }
    ...
    logging.info("Time-decay (max_len=50)   R@50=%.6f  ep=%s  t=%.0fs",
                 result["td_recall@50"],    result["td_best_epoch"],    result["td_total_train_sec"])
    logging.info("Time-aware Attention      R@50=%.6f  ep=%s  t=%.0fs  Δ=%+.6f",
                 result["attn_recall@50"],  result["attn_best_epoch"],  result["attn_total_train_sec"],
                 result["attn_vs_td_delta"])
    logging.info("Gated (TD+Attn)           R@50=%.6f  ep=%s  t=%.0fs  Δ=%+.6f",
                 result["gated_recall@50"], result["gated_best_epoch"], result["gated_total_train_sec"],
                 result["gated_vs_td_delta"])
    for bk in ("le5", "6to20", "gt20"):
        cnt = result["td_bucket_counts"].get(bk, 0)
        logging.info(
            "  %-6s (n=%d)  td=%.6f  attn=%.6f (Δ%+.6f)  gated=%.6f (Δ%+.6f)",
            bk, cnt,
            result["td_bucket_recall@50"][bk],
            result["attn_bucket_recall@50"][bk],  result["bucket_delta_attn_vs_td"][bk],
            result["gated_bucket_recall@50"][bk], result["bucket_delta_gated_vs_td"][bk],
        )
    logging.info("Threshold(≥+0.001): attn=%s  gated=%s",
                 result["meets_threshold_attn"], result["meets_threshold_gated"])
    logging.info("Unique hits: td=%d  attn=%d  gated=%d  all_three=%d  only_td=%d  only_attn=%d  only_gated=%d",
                 result["td_total_hits"], result["attn_total_hits"], result["gated_total_hits"],
                 result["all_three_hit"], result["only_td"], result["only_attn"], result["only_gated"])
    logging.info("==============================")
    return result
