#!/usr/bin/env python3
"""Smoke test: Text + Time-decay Mean Pooling + Item Popularity Bucket Two-Tower (1 epoch).

Architecture extends Time-decay Text+MP τ=0.15 (current best model) with a small
item popularity bucket embedding fused into the item tower.

User tower (unchanged from time-decay baseline):
  user_id embedding + time-decay weighted history item-id embedding pooling
  decay_rate = 0.8, weight_k = decay_rate^(seq_len-1-k)

Item tower (new):
  item_id embedding + frozen text embedding projection + popularity_weight * popularity_bucket_emb
  popularity buckets based on train interaction count per item:
    bucket 0: count <= 5     (long tail / rarely interacted)
    bucket 1: 6 <= count <= 20
    bucket 2: 21 <= count <= 100
    bucket 3: count > 100    (head items)

The popularity bucket embedding has 4 rows × embedding_dim columns.
popularity_weight (default 0.1) keeps it as an auxiliary signal, not overriding id/text.

Motivation: item popularity is a simple but informative signal.
Head items (>100 interactions) often have richer collaborative signal baked in via
the item_id embedding, while long-tail items rely more on text. The bucket embedding
lets the model learn a separate bias for each popularity tier without any manual engineering.

Decision thresholds (1epoch limited valid Recall@50):
  >= 0.116000  → suggest 5epoch (wait for user approval)
  0.114420–0.116000 → small improvement, record only
  < 0.114420   → no improvement, stop
"""
...
POPULARITY_BUCKETS = [
    (0, "count <= 5",        lambda c: c <= 5),
    (1, "6 <= count <= 20",  lambda c: 6 <= c <= 20),
    (2, "21 <= count <= 100",lambda c: 21 <= c <= 100),
    (3, "count > 100",       lambda c: c > 100),
]
NUM_POPULARITY_BUCKETS = 4
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Smoke: Text + Time-decay + Item Popularity Two-Tower.")
...
def require_config(config: dict[str, Any]) -> None:
    ...
    if not bool(config["use_item_popularity"]):
        raise ValueError("use_item_popularity must be true for this script.")
    pw = float(config["popularity_weight"])
    if pw <= 0:
        raise ValueError(f"popularity_weight must be > 0, got {pw}.")
...
def build_item_popularity_buckets(train_df: pd.DataFrame, num_items: int) -> tuple[np.ndarray, dict[int, int]]:
    """Assign each item a popularity bucket index based on train interaction count.

    Items with zero train interactions default to bucket 0 (count <= 5).
    Returns (bucket_array of shape [num_items], bucket_counts dict).
    """
    counts = train_df.groupby("item_idx").size()
    buckets = np.zeros(num_items, dtype=np.int64)  # default: bucket 0 (unseen / long tail)
    for item_idx, count in counts.items():
        ...
        if c <= 5:
            ...
            buckets[item] = 3
    bucket_counts = {int(b): int((buckets == b).sum()) for b in range(NUM_POPULARITY_BUCKETS)}
...
class TextTimeDecayPopularityTwoTower(nn.Module):
    """Text + Time-decay Mean Pooling + Item Popularity Bucket Two-Tower.

    User tower: identical to Time-decay Text+MP τ=0.15 (no change).
    Item tower: item_id_emb + text_proj + popularity_weight * pop_bucket_emb.

    The popularity bucket embedding is a new nn.Embedding(4, embedding_dim).
    popularity_weight (default 0.1) keeps popularity as a minor auxiliary signal.
    """
    ...
    def __init__(
        ...
        self.item_pop_embedding = nn.Embedding(NUM_POPULARITY_BUCKETS, embedding_dim)
        ...
        self.register_buffer("_item_pop_bucket", item_pop_buckets.long(), persistent=True)
        ...
        nn.init.normal_(self.item_pop_embedding.weight, mean=0.0, std=0.02)
    ...
    def time_decay_mean_history_embedding(
        ...
        decay_weights = self.decay_rate ** (seq_len - 1 - positions)
        decay_weights = decay_weights.unsqueeze(0).unsqueeze(-1)
        mask = valid_mask.unsqueeze(-1).to(history_emb.dtype)
        weighted_emb = history_emb * mask * decay_weights
        summed = weighted_emb.sum(dim=1)
        weight_sum = (mask * decay_weights).sum(dim=1).clamp_min(1e-8)
    ...
    def _item_prenorm(self, item_idx: torch.Tensor) -> torch.Tensor:
        ...
        pop_bucket = self._item_pop_bucket[item_idx]
        pop_emb = self.item_pop_embedding(pop_bucket)
        return id_emb + txt_proj + self.popularity_weight * pop_emb
...
def compute_logits(
    model: TextTimeDecayPopularityTwoTower,
    user_idx: torch.Tensor,
    item_idx: torch.Tensor,
    history_item_idx: torch.Tensor,
    temperature: float,
...
def train_epoch(
    ...
    for batch_idx, (user_idx, item_idx, history_item_idx) in enumerate(train_loader):
        ...
        loss, sim_min, sim_max, effective_batch_size = train_one_step(
            model, optimizer, user_idx, item_idx, history_item_idx, float(config["temperature"])
        )
        if batch_idx == 0:
            first_min = sim_min
            first_max = sim_max
...
def encode_all_items_cpu(model: TextTimeDecayPopularityTwoTower, num_items: int, device: torch.device) -> torch.Tensor:
    model.eval()
    chunks = []
    with torch.no_grad():
...
def build_model(
    ...
    train_df: pd.DataFrame,
    device: torch.device,
) -> tuple[TextTimeDecayPopularityTwoTower, dict[int, int]]:
    text_emb = torch.from_numpy(np.load(Path(config["item_text_embedding_path"])).astype(np.float32))
    has_text = torch.from_numpy(np.load(Path(config["item_has_text_path"])).astype(np.float32))
    logging.info("text_emb: shape=%s", tuple(text_emb.shape))
    logging.info(
        "has_text: %d/%d items have real text (%.1f%%)",
    ...
    num_items = int(stats["n_items"])
    pop_buckets_np, bucket_counts = build_item_popularity_buckets(train_df, num_items)
    pop_buckets_tensor = torch.from_numpy(pop_buckets_np)
    for b, (_, desc, _) in enumerate(POPULARITY_BUCKETS):
        logging.info("popularity bucket %d (%s): %d items", b, desc, bucket_counts[b])
    model = TextTimeDecayPopularityTwoTower(
        ...
        item_pop_buckets=pop_buckets_tensor,
        popularity_weight=float(config["popularity_weight"]),
    ).to(device)
    logging.info(
        "TextTimeDecayPopularityTwoTower trainable params: %s  decay_rate=%s  popularity_weight=%s",
        sum(p.numel() for p in model.parameters() if p.requires_grad),
        config["decay_rate"],
        config["popularity_weight"],
    )
    return model, bucket_counts
...
def train(config: dict[str, Any]) -> dict[str, Any]:
    ...
    logging.info(
        "device=%s embedding_dim=%s batch_size=%s lr=%s temperature=%s pooling_type=%s decay_rate=%s popularity_weight=%s",
        device,
    ...
    model, bucket_counts = build_model(config, bundle.stats, bundle.train_df, device)
    ...
    for epoch in range(1, int(config["epochs"]) + 1):
        ...
        valid_metrics = evaluate_with_oom_retry(
            model, bundle.valid_df, train_history_matrix, train_seen, config, bundle.stats, device, split_name="valid"
        )
        ...
        logging.info(
            ...
            epoch, train_loss, valid_metrics["recall@50"], valid_metrics["ndcg@50"], valid_metrics["mrr@50"], epoch_time,
        )
        logging.info(
        ...
        if valid_metrics["recall@50"] > best_valid_recall:
            ...
            save_checkpoint(
                ...
                model, config, bundle.stats, epoch, "valid_recall@50", best_valid_recall,
            )
    ...
    smoke_summary = {
        ...
        "pooling_type": str(config["pooling_type"]),
        "popularity_weight": float(config["popularity_weight"]),
        "popularity_bucket_counts": bucket_counts,
        "output_dir": str(output_dir),
    }
    write_json(output_dir / "popularity_smoke_results.json", smoke_summary)
    logging.info("Popularity smoke complete: %s", json.dumps(smoke_summary, ensure_ascii=False, sort_keys=True))
    return summary
