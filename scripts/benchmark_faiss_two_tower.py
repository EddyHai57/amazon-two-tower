#!/usr/bin/env python3
"""Faiss ANN offline benchmark — final Two-Tower (Text+Time-decay Mean Pool, τ=0.15).

Benchmark target: standalone full test Recall@50 = 0.078315.
NOT the four-channel weighted RRF value (0.104776).

Index types:
  1. FlatIP   — exact retrieval, correctness baseline
  2. IVFFlat  — nprobe = 16 / 32 / 64
  3. HNSW     — M=32, efSearch = 64 / 128  (skipped if METRIC_INNER_PRODUCT unsupported)

Output: outputs/faiss_two_tower_benchmark/
"""
...
NLIST = 1024            # IVF coarse clusters; 39 × 1024 = 39,936 ≤ 153,977 (no Faiss warning)
...
CONFIG_PATH = Path("configs/two_tower_movies_tv_5core_text_time_decay_mean_pool_20epoch.yaml")
CHECKPOINT_PATH = Path("outputs/text_time_decay_mean_pool_20ep/checkpoints/best_model.pt")
OUTPUT_DIR = Path("outputs/faiss_two_tower_benchmark")

BATCH_SIZE_EMBED = 8192
BATCH_SIZE_SEARCH = 8192
FAISS_THREADS = 8

EXPECTED_R50 = 0.07831490321670997   # full test, from metrics_test_full.json
...
def compute_recall_metrics(
    ...
    for i in range(n):
        ...
        cands = topk_indices[i].tolist()      # numpy int64 → Python int list (fast)
...
def main() -> None:
    ...
    test_history, _ = td.build_history_matrix(test_hist_frame, num_users, history_max_len)
    ...
    output: dict[str, Any] = {
        ...
        "model": "Text+Time-decay Mean Pool Two-Tower tau=0.15 decay_rate=0.8",
    ...
    # Inline benchmark report (outputs/)
...
def _write_inline_report(
    ...
    flat_tp = flat.get("throughput_users_per_sec", 0)
    flat_lat = flat.get("avg_latency_ms", 0)
    flat_search = flat.get("search_sec", 0)

    lines = [
        "# Faiss Two-Tower Offline Benchmark",
        "",
    ...
    ivf_info = f"IndexIVFFlat  nlist={meta['nlist']}  nprobe values={meta['nprobe_values']}"
    hnsw_note = ("HNSW skipped: faiss.IndexHNSWFlat does not support "
    ...
    lines += [
        ...
        f"- Offline benchmark only. Not representative of online serving latency.",
        f"- HNSW: {hnsw_note}",
        "",
        "## Environment",
        f"- Python: {env['python']}",
